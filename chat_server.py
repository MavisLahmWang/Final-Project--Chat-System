...
# ==============================================================================
                ...
                for wd in words:
                    index_msg = text_proc(msg_text, from_name)
                    if wd not in self.indices:
                        self.indices[wd] = [index_msg]
                    else:
                        self.indices[wd].append(index_msg)
                ...
                for g in the_guys:
                    ...
                    new_msg = "[" + from_name + "] " + msg_text
                    # send the message to others
                    mysend(to_sock, json.dumps(
                        {"action": "exchange", "message": new_msg, "from": from_name}))
...
# ==============================================================================
                ...
                from_name = self.logged_sock2name[from_sock]
                poem_num = int(msg["target"].strip("p"))
                try:
                    poem_lst = self.sonnet.get_poem(poem_num)
                    poem = ""
...
# ==============================================================================
                ...
                term = msg["target"]
                try:
                    search_rslt = str(self.indices[term])
                except:
                    search_rslt = ""
